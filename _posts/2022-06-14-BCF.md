...
Finally, I experimented with a range of different sktime models (Naive, AutoARIMA, AutoETS, ManualETS), and found the ManualETS estimator to be the best peforming. With Calais data, this model returns a mean squared error around 22,546 and a mean absolute percentage error of 20%. With Buffalo data, the model performs even better with a mean squared error around 60,511 and a mean absolute percentage error of 6%. The plots below show the performance of the ManualETS estimator against the baseline and test data...first with the Calais border crossing, and second with the Buffalo border crossing. 
